...
const filterBtn = (active: boolean, darkMode: boolean) =>
  active
    ? darkMode
      ...
      : "bg-[var(--sys-surface)] border border-[var(--sys-border)] shadow-ios-sm text-[var(--sys-blue)] shadow-xs"
      ...
      <div className="p-4 sm:p-5 bg-[var(--sys-surface)] rounded-[20px] border border-[var(--sys-border)] shadow-ios-sm">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-[var(--sys-blue)]/10 text-[var(--sys-blue)] rounded-[14px] flex items-center justify-center font-bold flex-shrink-0">
              <CalendarIcon size={18} />
            </div>
            <div>
              <h3 className="font-semibold text-[var(--sys-text)]">Lịch Kinh Tế</h3>
              <p className="text-sm text-[var(--sys-text-secondary)]">Chỉ số USD quan trọng</p>
            </div>
          </div>

          <div className="flex items-center gap-2">
            {/* Period filter */}
            <div className={`flex ${darkMode ? "bg-[var(--sys-surface)] border border-[var(--sys-border)]" : "bg-[var(--sys-surface-2)]"} p-0.5 rounded-[10px]`}>
              <button onClick={() => setCalendarPeriodFilter("DAY")}
                className={`px-3 py-1 text-xs font-medium rounded-[8px] transition-all cursor-pointer ${calendarPeriodFilter === "DAY" ? (darkMode ? "bg-[var(--sys-surface-2)] border border-[var(--sys-border)] text-[var(--sys-blue)]" : "bg-[var(--sys-surface)] border border-[var(--sys-border)] shadow-ios-sm text-[var(--sys-blue)]") : "text-[var(--sys-text-secondary)]"}`}>Hôm nay</button>
              <button onClick={() => setCalendarPeriodFilter("WEEK")}
                className={`px-3 py-1 text-xs font-medium rounded-[8px] transition-all cursor-pointer ${calendarPeriodFilter === "WEEK" ? (darkMode ? "bg-[var(--sys-surface-2)] border border-[var(--sys-border)] text-[var(--sys-blue)]" : "bg-[var(--sys-surface)] border border-[var(--sys-border)] shadow-ios-sm text-[var(--sys-blue)]") : "text-[var(--sys-text-secondary)]"}`}>Tuần này</button>
            </div>
            ...
            <button onClick={syncCalendar} className="p-2 rounded-[10px] bg-[var(--sys-surface-2)] border border-[var(--sys-border)] cursor-pointer active:scale-90 transition-transform" title="Cập nhật">
              <RefreshCw size={14} className={refreshingCalendar ? "animate-spin text-[var(--sys-blue)]" : "text-[var(--sys-text-secondary)]"} />
            </button>
          </div>
        </div>
        ...
        {loadingCalendar ? (
          <div className="py-16 text-center space-y-3">
            <RefreshCw className="animate-spin text-[var(--sys-blue)] mx-auto" size={28} />
            <p className="text-[var(--sys-text-secondary)]">Đang quét Forex Factory...</p>
          </div>
          ...
          <div className="space-y-6">
            {groupedEventsByDay.map(([dayName, events]) => (
              <div key={dayName} className="space-y-3">
                <h4 className="text-xs font-semibold uppercase tracking-wider text-[var(--sys-blue)] bg-[var(--sys-blue)]/10 px-2.5 py-1 rounded-[8px] inline-block">{dayName}</h4>
                <div className="flex flex-col gap-2">
                  {events.map((ev, index) => {
                    ...
                    return (
                      <div key={`${dayName}-${index}`} className="flex items-center gap-3 p-3 bg-[var(--sys-surface)] rounded-[14px] border border-[var(--sys-border)] shadow-ios-sm hover:bg-[var(--sys-surface-2)] transition-colors">
                        {/* Time & Impact */}
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-sm text-[var(--sys-text)] truncate">{ev.title}</p>
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <span className="text-[10px] bg-[var(--sys-text)]/5 px-1.5 py-0.5 rounded text-[var(--sys-text-secondary)] uppercase font-mono">{ev.country}</span>
                            <span className="text-[10px] text-[var(--sys-text-secondary)]">{timezoneOffsetStr}</span>
                          </div>
                        </div>
                        {/* Desktop Stats */}
                        <div className="hidden md:flex flex-col items-end gap-1 border-l border-[var(--sys-border)]/20 pl-3">
                          <div className="flex gap-3 text-xs text-[var(--sys-text-secondary)]">
                            <span>DB: <strong className="text-[var(--sys-text)]">{ev.forecast || "-"}</strong></span>
                            <span>Trước: <strong className="text-[var(--sys-text)]">{ev.previous || "-"}</strong></span>
                          </div>
                          <div className="text-xs">
                            <span className="font-semibold text-[var(--sys-blue)] uppercase">TT: </span>
                            <strong className="text-[var(--sys-blue)]">{ev.actual || "Đợi tin"}</strong>
                          </div>
                        </div>
                        {/* Mobile Stats */}
                        <div className="flex md:hidden flex-col items-end min-w-[55px]">
                          <div className="text-[10px] text-[var(--sys-text-secondary)]"><span className="opacity-60">DB:</span> <strong>{ev.forecast || "-"}</strong></div>
                          <div className="text-xs font-bold text-[var(--sys-blue)]"><span className="opacity-60 text-[9px] uppercase">TT:</span> {ev.actual || "-"}</div>
                        </div>
                      </div>
                    );
                  })}
