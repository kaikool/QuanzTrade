...
export function IOSTabBar({ currentTab, setCurrentTab }: IOSTabBarProps) {
  return (
    <nav
      id="ios-bottom-nav"
      className="md:hidden fixed z-40 inset-x-0 bottom-0 flex items-center justify-around px-2 bg-[var(--ios-surface)]/70 border border-[var(--ios-separator)] border-b-0 rounded-t-[34px] shadow-ios-glass"
      style={{
        bottom: "0",
        minHeight: "82px",
      ...
      {tabs.map(({ id, label, Icon }) => {
        ...
        return (
          <button
            ...
            className={`relative flex flex-col items-center justify-center flex-1 min-w-0 h-full gap-0.5 transition-colors duration-150 cursor-pointer ${
          ...
          >
            ...
            <span className={`text-[10px] leading-tight tracking-normal truncate max-w-full ${isActive ? "font-semibold" : "font-[400]"}`}>
              {label}
            </span>
          </button>
        );
