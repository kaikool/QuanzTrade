    ...
    <div className="h-full flex flex-col md:h-[calc(100vh-140px)]" id="journal-standalone-section">
      ...
      <div className="space-y-3 mb-4 shrink-0">
        ...
        <div className="grid grid-cols-[minmax(92px,0.85fr)_minmax(104px,1fr)_minmax(92px,0.8fr)_40px] gap-2 min-w-0">
          ...
          <select value={selectedPairFilter} onChange={(e) => setSelectedPairFilter(e.target.value)} className="min-w-0 h-10 ios26-glass border-0 px-3 rounded-[12px] focus:outline-none focus:ring-2 focus:ring-[var(--ios-blue)]/50 cursor-pointer text-[var(--ios-label)] font-bold text-[13px] shadow-sm appearance-none truncate">
            <option value="ALL">Cặp (Tất cả)</option>
            {uniquePairs.filter(p => p !== "ALL").map(p => <option key={p} value={p}>{p}</option>)}
          </select>

          <select value={selectedStatusFilter} onChange={(e) => setSelectedStatusFilter(e.target.value)} className="min-w-0 h-10 ios26-glass border-0 px-3 rounded-[12px] focus:outline-none focus:ring-2 focus:ring-[var(--ios-blue)]/50 cursor-pointer text-[var(--ios-label)] font-bold text-[13px] shadow-sm appearance-none truncate">
          ...
          <div className="flex-1 flex flex-col overflow-hidden relative z-20 min-h-0 w-full md:w-auto">
            ...
            <div className="flex items-center justify-between p-4 border-b border-[var(--ios-separator)]/50 shrink-0">
              <div className="flex items-center gap-3 min-w-0">
                <button onClick={() => setSelectedTrade(null)} className="md:hidden w-8 h-8 flex items-center justify-center ios26-glass text-[var(--ios-label)] rounded-full active:scale-90 transition-transform">
                  <ArrowLeft size={18} />
                </button>
